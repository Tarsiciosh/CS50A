...
class Minesweeper():
    ...
    def nearby_mines(self, cell):
        ...
        # T: cell is a tuple? cell[0] means row | cell[1] means col?
        # T: range doenst count the upper limit
...
class MinesweeperAI():
            ...
            for sentence1 in self.knowledge:
                for sentence2 in self.knowledge:
                    ...
                    if sentence1.cells < sentence2.cells:
                        if sentence1.count > 0 and sentence2.count > 0:                        
                            addNewSentece = True
                            newSentece.cells = sentence2.cells.difference(sentence1.cells)
                            newSentece.count = sentence2.count - sentence1.count
                            sent1 = sentence1
                            sent2 = sentence2
            if addNewSentece:
    ...
    def make_random_move(self):
        ...
        candidateCells = candidateCells.difference(self.moves_made)
        candidateCells = candidateCells.difference(self.mines)

        if candidateCells != set():
            return candidateCells.pop() # it is not random
        return None
